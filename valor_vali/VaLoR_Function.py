...
def Pacbio_prodce_ref_alt_long_dis_dup(ref,flank_length,info,block,alt_status):
	...
	if alt_status=='het':
		return [upper_string(ref_seq),upper_string(ref_seq),upper_string(alt_seq)]
	else:
		return [upper_string(ref_seq),upper_string(alt_seq),upper_string(alt_seq)]
def calcu_eu_dis_long_dis_dup(global_list,svelter_list,plt_figure_index,PacVal_score_hash,info,SV_index,SV_Length_Cff):
	#info=[k1,k2]+k3
	[lenght_cff,dots_num_cff,clu_dis_cff, point_dis_cff, simple_dis_cff, invert_base, dict_opts, out_path, out_file_Cannot_Validate, sample_name, start, delta, bam_in, ref, chromosomes, region_QC_Cff, min_length, min_read_compare, case_number, qc_file_name]=global_list
	[PacVal_file_in,PacVal_file_out]=svelter_list
	window_size=10
	...
	flank_length=flank_length_calculate(bps)
	if bps_check(bps,chromosomes)==0:
		abnormal_blocks=abnormal_block_detect(info,flank_length)
		...
		for x in abnormal_blocks:
			if abnormal_blocks.count(x)==1:
				...
				alt_status='homo'
			if block_len_calcu(info,flank_length,x)>SV_Length_Cff: continue #to be continued
			else:
				seqs=Pacbio_prodce_ref_alt_long_dis_dup(ref,flank_length,info,x,alt_status)
				bp_let_hash=bp_let_to_hash(info,flank_length)
				info_new=['a/a','a/a']+[info[2],bp_let_hash[x[0]][1],bp_let_hash[x[2]][0]]
				all_reads=chop_pacbio_read_simple_short(bam_in,info_new,flank_length)
				if not all_reads[0]==[]:
					all_reads_new=read_hash_minimize(all_reads)
					[read_hash,miss_hash,name_hash]=[all_reads_new[0],all_reads_new[1],name_hash_modify(all_reads_new[2])]
					...
					remove_files_short(txt_file)
	return PacVal_score_hash
def calcu_eu_dis_bed_simpleSV(global_list,svelter_list,plt_figure_index,PacVal_score_hash,km):
	[lenght_cff,dots_num_cff,clu_dis_cff, point_dis_cff, simple_dis_cff, invert_base, dict_opts, out_path, out_file_Cannot_Validate, sample_name, start, delta, bam_in, ref, chromosomes, region_QC_Cff, min_length, min_read_compare, case_number, qc_file_name]=global_list
	[PacVal_file_in,PacVal_file_out]=svelter_list
	info=km[3:5]+km[:3]
	[k1,k2,k3]=[info[0],info[1],info[2:]]
	...
	if bps_check(bps,chromosomes)==0:
		...
		if len(all_reads[0])>10:
			PacVal_score_hash=calcu_eu_dis_complex_short(global_list,svelter_list,plt_figure_index,PacVal_score_hash,info,km[-2])
		else:
			PacVal_score_hash=calcu_eu_dis_simple_long(global_list,svelter_list,plt_figure_index,PacVal_score_hash,info,km[-2])
	return PacVal_score_hash
def calcu_eu_dis_bed_simple_ins(global_list,svelter_list,plt_figure_index,PacVal_score_hash,km):
	[lenght_cff,dots_num_cff,clu_dis_cff, point_dis_cff, simple_dis_cff, invert_base, dict_opts, out_path, out_file_Cannot_Validate, sample_name, start, delta, bam_in, ref, chromosomes, region_QC_Cff, min_length, min_read_compare, case_number, qc_file_name]=global_list
	[PacVal_file_in,PacVal_file_out]=svelter_list
	info=km[3:5]+km[:3]
	...
	if bps_check(bps,chromosomes)==0:
		bl_len_hash=bl_len_hash_calculate(bps,ref_sv)
		all_reads=chop_pacbio_read_simple_insert(bam_in,info,flank_length,insert_seq)
		if len(all_reads[0])>10:
			...
			if not insert_seq.count('n')==len(insert_seq): 
				...
				PacVal_score_hash=calcu_eu_dis_ins_long(global_list,svelter_list,plt_figure_index,PacVal_score_hash,km[3:5]+km[:3],km[-2],km[-1])
	return PacVal_score_hash